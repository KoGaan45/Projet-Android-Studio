...
class WebServiceStatsNettoyeur {
    ...
    fun call(session : Int, signature : Long) : Joueur? {
        return try {
            ...
            Log.d(TAG,"http://51.68.124.144/nettoyeurs_srv/stats_nettoyeur.php?session=$session&signature=$signature")
            ...
            return Joueur(session,signature,messagesXML.item(0).textContent.toString())
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
