...
class WebServiceFrappeCible {
    private val TAG = "WSCreationFrappeCible"

    fun call(session : Int, signature : Long, cibleId: Int) : String? {
        return try {
            val url = URL("http://51.68.124.144/nettoyeurs_srv/frappe_cible.php?session=$session&signature=$signature&cible_id=$cibleId")
            ...
            Log.d(TAG, "Thread Frappe de cible : status $status")
            ...
            val outcome = messagesXML.item(0).textContent.toString()
            val detecter = messagesXML.item(0).textContent.toString()

            return "Outcome: $outcome | Detecter: $detecter"
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
